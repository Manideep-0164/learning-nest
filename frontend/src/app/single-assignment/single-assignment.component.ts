...
export class SingleAssignmentComponent implements OnInit {
  ...
  getAssignemnts() {
    ...
    this.http
      .get(
        `${environment.API_URL}/api/assignment/${assignmentId}/student/${
          this.globalService.getStudentLoginDetails()?.id
        }`,
        { headers }
      )
      .subscribe((res: any) => {
  ...
  handleAddSubmission() {
    ...
    this.http
      .post(`${environment.API_URL}/api/submission`, submission, { headers })
      .subscribe((res) => {
        console.log(res);
        this.messageService.add({
          ...
          severity: 'success',
          summary: 'Submitted',
          detail: 'Assignment successfully submitted',
        });
      });
  }
